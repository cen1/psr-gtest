...
public:
	...
	void CalculatePSRNew(vector<TupledPlayerValue> team1, vector<TupledPlayerValue> team2);
...
public:
	...
	double GetPlayerValue() {
		...
		//Expected max values
		const float c_MaxAverageWL = 2;
		const float c_MaxAverageKD = 2.2;
		const float c_MaxAverageAD = 2.2;
		const float c_MaxAverageCreepKills = 250;
		const float c_MaxAverageNeutralKills = 80;
		const float c_MaxAverageCreepDenies = 10;
		const float c_MaxAverageRaxKills = 1.2;
		const float c_MaxAverageTowerKills = 2.2;
	...
	double GetBalanceValue() {
		double mean = (m_Rating + GetPlayerValue()) / 2;
		return mean;
	}
